...
与我方实际生产环境中不同。实际生产环境是`oracle jre 8`，截止`2020-02-14`的最新版是`241`。所以，应使用`centos`建立新的镜像。现在下载的`java8`仍可用于实验。
...
```bash
# 使用默认文件名`Dockerfile`
sudo docker build -t yxy/centos/jdk8 .
# 通过`-f`指定`Dockerfile`文件名
sudo docker build -f <Dockerfile文件名> -t yxy/centos/jdk8 .
```
...
该程序只有一个`jar`包，没有任何依赖。程序接收最多两个参数，第一个参数是循环次数，第二个参数是需要输出的信息字符串。如果不提供参数，则打印一条信息后直接返回。
...
public class Simple {
    public static void main(String[] args) throws InterruptedException {
        ...
        if (argsCount == 0) {
            System.out.println("This is the message returned from SIMPLE without any parameter.");
            return;
        } else if (argsCount > maxArgs) {
            System.out.println("No more than " + maxArgs + " parameters for SIMPLE!");
            return;
        }
...
```bash
sudo docker build -f yxy-simple.dockerfile -t yxy/simple .
docker images
```
...
是否设置`LC_ALL`对输出中文无影响。

### 5.2带依赖的程序simpleLog

#### 5.2.1功能

增加了对`log4j2`的依赖，将直接打印到控制台的输出转为日志输出，以及列出指定目录下的文件名，其它与上一节中最简单的程序没有区别。
...
# 设置工作目录的作用是，外部检查当前目录（.或./），就是工作目录。
# 另外，log4j2配置文件如果使用相对路径，如果logs存储日志文件，将在工作目录下建立。
...
```bash
sudo docker build -f yxy-simple-log.dockerfile -t yxy/simple-log .
```

在构造文件中，指明的是复制目录到容器。目录中包含4个文件，即`simpleLog-1.0.jar`，两个`log4j2`的`jar`包，以及`log4j2`的配置文件。
...
容器上当前目录是`/opt/simpleLog`。在其下有日志目录`logs`。如果构建镜像时，未明确指定工作目录，则以上返回中，当前目录将是`/opt`，而其下将有`/opt/logs`和`/opt/simpleLog`两个目录。此时，由于命令指定的容器目录`/opt/simpleLog/logs`不存在，目录挂载将失败，但**不会报错！**

在宿主机目录与容器目录均正确时，挂载生效。`/home/jason/docker/logs`下将产生文件`app.log`和`error.log`，与日志配置相符。`app.log`内容也正确，此处忽略。
...
相当于在没有相应jar包时执行了`java -jar simpleLog-1.0.jar`。
...
挂载参数不仅可以挂载目录，还可以直接挂载文件，从而解决配置文件的问题。

仍以`simpleLog`为例，其目录中包含4个文件，即`simpleLog-1.0.jar`，两个`log4j2`的`jar`包，以及`log4j2.xml`。如果在构建镜像前将`log4j2.xml`删除，则不包含日志配置文件的镜像仍能正常运行，但不会有任何输出，因为默认输出是容器内的控制台。
...
1. [Docker容器之最小JDK基础镜像](https://cloud.tencent.com/developer/article/1188404)：`Dockerfile`中设置了工作目录，使用`alpine-glibc`镜像为基础，最终生成的镜像大小仅`122M`。包含去除无用的jar包。
...
2. [Docker入门教程](http://www.docker.org.cn/book/docker)：极为精简的教程，可在15分钟内看完、练完，初步感觉`docker`是咋回事。
